"""
This script classifies sleep into light, deep and REM sleep.
...
###
# Feature extraction and sleep classification
###
def sleep_classification(
    inactive_segments_processed: list,
    epoch_length: float,
    window_length: float,
    fs: float,
    dict_labels: dict,
):
    """Function for classifying sleep segments into REM, light and deep NREM.

    Args:
        inactive_segments_processed (list): continous data for each segment with time and value
        epoch_length (float): length of an epoch to classify
        window_length (float): _description_
        fs (float): _description_
        dict_labels (dict): _description_
    """
    ...
    (
        ...
        rem_features_mean,
        rem_feature_std,
    ) = fun1.extract_features(
        inactive_segments_processed, epoch_length, window_length, fs
    )
    msg = "extracted the features for the segments, shape of the dataset is %s" % str(
        all_features.shape
    )
    print(msg)
    ...
    (
        labels_rem,
        nrem_probability_matrix,
        optimization_fig,
        silhouettes_rem,
        optimizing_threshold_rem,
        rem_feature_threshold,
    ) = fun1.compute_and_optimize_rem_cluster(
        all_features, dict_labels, all_features_mean, all_features_std
    )
    ...
    (
        segment_labels,
        silhouettes_nrem,
        optimizing_threshold_nrem,
        optimization_silhouette_fig,
    ) = fun1.compute_and_optimize_nrem_cluster(
        all_features, dict_labels, labels_rem, nrem_probability_matrix, optimization_fig
    )
    msg = "labeled DS and LS epochs."
    print(msg)

    return all_features, epoch_times, rem_features, segment_labels, optimization_silhouette_fig


###
# Data reading and output saving
###
def main_task(file_name):
    """
    This function executes the main task of WUCSS. This contains format-dependent operations.
    ...
    wucss_min_inactive_duration = (
        1  # minimum that the inactive interval should have (in seconds)
    )
    ...
    epoch_length = 4.0  # Parameters for epoch creation
    window_length = 4.0  # Parameters for epoch creation
    dict_labels = {
        0: light_sleep_itv_name,
        1: deep_sleep_itv_name,
        2: new_rem_itv_name,
    }  # label and name for the different states
    dict_postprocessing = {
        new_rem_itv_name: (12.0, 0.0),
        light_sleep_itv_name: (4.0, 0.0),
        deep_sleep_itv_name: (12.0, 0.0),
    }  # post-processing parameters, merging window (in seconds)
    rec_time = [
        0.0,
        24.0,
    ]  # time period with start and end in hours that should be used for the transition matrix
    tm_interval_list = [
        include_itv_name,
        new_rem_itv_name,
        light_sleep_itv_name,
        deep_sleep_itv_name,
    ]  # interval names that will be used for the transition matrix
    ...
    if duration[inactive_itv_name] < wucss_min_inactive_duration:
        msg = (
            "duration of inactive interval is %s seconds and therefore below the required duration, skipping recording"
            % (duration)
        )
        print(msg)
        return
    ...
    conservative_inactive_interval = nc.exclude_interval_from_other_interval(
        nexfile, inactive_itv_name, activity_bout_name
    )

    # get continuous data limited to time of interval
    inactive_segments_processed, fs = fun1.get_continous_during_interval(
        nexfile,
        feature_channel_names,
        conservative_inactive_interval,
        processing=True,
        min_length=10,
        time_threshold=0,
    )
    msg = "extracted continous file of %s for %s" % (
        str(feature_channel_names),
        inactive_itv_name,
    )
    print(msg)
    ...
    (
        ...
        inactive_segments_processed, epoch_length, window_length, fs, dict_labels
    )

    # postprocessing steps for intervals
    df_intervals = fun1.postprocessing_labels(
        nexfile,
        segment_labels,
        epoch_times,
        dict_labels,
    ...
    optimization_silhouette_cluster_fig = fun1.save_clustering_pictures(
        res_folder,
        all_features,
        rem_features,
        segment_labels,
    ...
    fun1.calculate_transition_matrices(
        nexfile,
        res_folder,
        tm_interval_list,
        optimization_silhouette_cluster_fig,
        df_intervals,
        rec_time,
    )
...
# Execute functions when script is run
